...
const getFileExtension = (FileName) => {
    ...
    uploadFile(file, setFileState, setProgress, async (data) => {
      if (data.status) {
        ...
        let upload = {
          ...
          type: "audio",
          thumbnail:"/undraw_listening_re_c2w0.svg",
          postType: `audio/${preview.type}`,
  ...
  return (
    ...
    <label htmlFor="file" className="block mx-auto mt-4 border-2 border-dotted max-w-sm h-auto py-6 px-4 rounded-lg cursor-pointer flex items-center justify-center flex-col cursor-pointer bg-gray-100">
        {
        (preview) ? (<audio src={preview.url} controls></audio>) :
...
Upload A Audio 🚀
  ...
  <input onChange={handleFile} className="hidden" type="file" accept="audio/*" id="file" />
